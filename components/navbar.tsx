...
function DesktopDropdown({ item, scrolled }: { item: NavDropdown; scrolled: boolean }) {
  ...
  return (
    <div className="relative" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      <button className={`flex items-center gap-1 text-sm font-medium transition-colors duration-200 ${scrolled ? "text-white hover:text-primary" : "text-foreground hover:text-primary"}`}>
        {item.name}
        <ChevronDown
  ...
  return (
    <div>
      <button
        ...
        className={`flex w-full items-center justify-between text-sm font-medium transition-colors ${scrolled ? "text-white hover:text-primary" : "text-foreground hover:text-primary"}`}
      >
        {item.name}
        <ChevronDown
